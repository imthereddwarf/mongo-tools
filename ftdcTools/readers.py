...
def read_meta(fn):
    ...
    """
    Read an Metadata from ftdc file. fn may be either a single metrics file, or a
    directory containing a sequence of metrics files.
    """
    ...
    if os.path.isdir(fn):
        for f in sorted(os.listdir(fn)):
            for meta in read_meta(os.path.join(fn, f)):
                yield meta
    ...
    # process file
        ...
        # traverse the file reading type 0 chunks
        while at < len(buf):
            try:
                chunk_doc = _read_bson_doc(buf, at)
                at += chunk_doc.bson_len
